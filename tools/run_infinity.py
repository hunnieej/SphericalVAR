...
def _resolve_t5_path(t5_path: str):
    if not t5_path:
        return t5_path
    required_files = ["tokenizer_config.json", "spiece.model", "tokenizer.json"]

    def has_all_files(path):
        return all(os.path.exists(os.path.join(path, file)) for file in required_files)

    if osp.isdir(t5_path) and not has_all_files(t5_path):
        snapshots_dir = osp.join(t5_path, "snapshots")
        if osp.isdir(snapshots_dir):
            candidates = [
                osp.join(snapshots_dir, d)
                for d in os.listdir(snapshots_dir)
                if osp.isdir(osp.join(snapshots_dir, d))
            ]
            candidates.sort(key=lambda d: osp.getmtime(d), reverse=True)
            for candidate in candidates:
                if has_all_files(candidate):
                    print(f"[Tokenizer] resolved snapshot path: {candidate}")
                    return candidate
    return t5_path
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_file", type=str, default="./tmp_360.jpg")
