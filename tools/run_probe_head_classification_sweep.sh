...
# Migrate legacy names that still contain "final" so skip logic stays consistent.
python - <<'PY'
from pathlib import Path
root = Path('probe_outputs/head_classification')
for path in sorted(root.iterdir(), key=lambda p: p.name):
    if 'final' not in path.name:
        continue
    new_name = path.name.replace('_final_step', '_step').replace('_final', '')
