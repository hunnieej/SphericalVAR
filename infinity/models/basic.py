...
# Import flash_attn's attention (fallback to None when unavailable)
try:
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        flash_available = flash_attn_func is not None
        self.using_flash = customized_flash_attn and flash_available
...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, ca_kv):
        ...
        if flash_attn_varlen_kvpacked_func is not None:
            ...
            dtype = kv_compact.dtype
            q = q_compact.view(B, Lq, self.num_heads, self.head_dim).transpose(
                1, 2
            )  # B, H, Lq, c
            kv = kv_compact
            max_len = int(max_seqlen_k)
            k = torch.zeros(
                B, max_len, self.num_heads, self.head_dim, device=device, dtype=dtype
            )
            ...
            for b in range(B):
                ...
                k[b, :length] = seg[:, 0]
                v[b, :length] = seg[:, 1]
                pad_mask[b, :length] = False
            k = k.transpose(1, 2)  # B, H, Lk, c
            v = v.transpose(1, 2)
            if pad_mask.any():
                attn_mask = torch.zeros(B, 1, Lq, max_len, device=device, dtype=q.dtype)
                attn_mask.masked_fill_(pad_mask[:, None, None, :], float("-inf"))
            else:
                attn_mask = None
            oup = (
                slow_attn(q, k, v, attn_mask=attn_mask, dropout_p=0)
                .transpose(1, 2)
                .reshape(B, Lq, -1)
            )
