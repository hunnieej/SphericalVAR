...
from .bsq_vae.interp_utils import interpolate_latent
            ...
            for si, (pt, ph, pw) in enumerate(vae_scale_schedule):
                ...
                if si < self.noise_apply_layers:
                    noise_apply_strength = (
                        np.random.randint(0, 100 * self.noise_apply_strength + 1) * 0.01
                    )
                    mask = (
                        torch.rand(*bit_indices.shape).to(device) < noise_apply_strength
                    )
                ...
                if si < len(vae_scale_schedule) - 1:
                    this_scale_input = interpolate_latent(
                        cum_var_input,
